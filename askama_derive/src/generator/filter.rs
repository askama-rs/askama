...
impl<'a> Generator<'a, '_> {
    ...
    fn visit_fmt_filter(
        ...
        let Expr::StrLit(ref fmt) = **fmt else {
            return Err(ctx.generate_error(r#"use filter fmt like `value|fmt("{:?}")`"#, node));
        };
        buf.write("askama::helpers::alloc::format!(");
        self.visit_str_lit(buf, fmt);
        buf.write(',');
...
fn no_arguments<'a, 'b>(
    ...
    match args {
        ...
        [_, arg, ..] => Err(ctx.generate_error(
            format_args!("filter `{name}` does not have any arguments"),
            arg.span(),
        )),
        _ => unreachable!(),
    }
            ...
            let Some(idx) = filter_args
                ...
                return Err(ctx.generate_error(
                    match filter_args.len() {
                        1 => fmt_left!("filter `{name}` does not have any arguments"),
                        _ => fmt_right!(
                            "filter `{name}` does not have an argument `{}`; {}",
    ...
    for (&arg, collected) in filter_args.iter().zip(collected_args) {
        if !is_argument_placeholder(collected) {
            ...
            *collected = default;
            ...
            return Err(ctx.generate_error(
                format_args!(
                    "filter `{name}` argument `{}` is missing{}",
...
impl fmt::Display for ItsArgumentsAre {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("; it's arguments are: (")?;
