...
use crate::{
    CharLit, ErrorContext, Level, Num, ParseResult, PathOrIdentifier, Span, StrLit, StrPrefix,
    WithSpan, char_lit, cut_error, filter, identifier, keyword, not_suffix_with_hash, num_lit,
    ...
    match &expr.inner {
        ...
        Expr::Path(path) => {
            if let [arg] = path.as_slice() {
                if !crate::can_be_variable_name(arg.name) {
    ...
    fn parse(i: &mut &'a str, level: Level<'_>) -> ParseResult<'a, WithSpan<'a, Expr<'a>>> {
        ...
        while let Some((suffix, i_before)) = opt(right.by_ref().with_taken()).parse_next(i)? {
            ...
            match suffix {
                ...
                Self::Call { args } => {
                    expr = WithSpan::new(Expr::Call(Box::new(Call { path: expr, args })), i_before)
                }
